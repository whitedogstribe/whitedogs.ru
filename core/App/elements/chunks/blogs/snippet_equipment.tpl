...
{set $rows = query('pb_table_data')->where('constructor_id', 35)->where('field_id', 406)->where('model_id', $resourceId)->where('model_type', 'PageBlocks\\App\\Models\\BlogPost')->whereNull('deleted_at')->orderBy('menuindex', 'asc')->get()}
{if $rows|length}
